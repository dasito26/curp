...
/**
 * Genera una CURP de acuerdo al nivel dado.
 * El nivel puede ser un valor del 1 y hasta el 10.
 *
 * Los niveles sirven para validar una CURP en el caso de que se tenga homonimia.
 *
 * @param {*} persona
 * @param {*} nivel
 * @return CURP generada
 */
function generarConNivel(persona, nivel = '') {
  validaNivel(nivel);
  let level = 0;
  if (nivel !== '') level = parseInt(nivel, 10) - 1;
  let curp = generar(persona);

  // De acuerdo con DOF https://www.dof.gob.mx/nota_detalle.php?codigo=5526717&fecha=18/06/2018#gsc.tab=0
  const letras = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J'];

  let posicionDiecisiete = curp.substring(16, 17);

  if (posicionDiecisiete === '0')
    posicionDiecisiete = parseInt(posicionDiecisiete, 10) + level;

  if (posicionDiecisiete === 'A') posicionDiecisiete = letras[level];

  curp = curp.substring(0, 16);
  curp += posicionDiecisiete;
  curp += agregaDigitoVerificador(curp);
...
/**
 * Valida que el valor de nivel sea uno correcto.
 * @param {*} nivel
 */
function validaNivel(nivel) {
  if ((nivel < 1 || nivel > 10 || isNaN(nivel)) && nivel !== '')
    throw new Error('El nivel debe ser un numero mayor que 0 y menor que 11');
}
...
curp.generarConNivel = generarConNivel;
curp.GENERO = GENERO;
curp.ESTADO = ESTADO;
module.exports = curp;
